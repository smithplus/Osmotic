...
swift scripts/make_install_steps.swift .   # the landing's install steps → docs/images/step-*.png (then cwebp) + step-1..3.mp4 (4-s loops)
...
- Landing hero: a looping video of one session (connect, tick two clips, download), made by `scripts/make_demo_video.swift` from demo-mode renders of the app plus a drawn pointer (no screen recording). `main.js` starts it after `load` (the poster paints first) and pauses it out of view; the picture itself is the control (a click, Enter or Space stops it), with no buttons or step readout on the page, and Reduce Motion leaves the poster.
...
- Landing videos: the hero (a session) and the Live feature (record, stop, modes) loop while in view, with no visible controls; the three install steps are clips that loop too (the drag onto Applications, the click on Open Anyway, the click on Connect; each ends where it starts, so the seam does not show). Files and Webcam stay stills (the hero already shows Files; the Webcam screen doesn't move).
