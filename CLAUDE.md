...
- Landing page (`site/`): same tokens as `Theme` in CSS (keys, LCD, panels with screws), materials from `site/textures/` (a 128-px grain tile on the plate, keys and pockets; brushed streaks on the metal panels) plus CSS-only screen textures (scanlines on the LCDs, a dot grid on the readout tables, a punched speaker grille on the download plate), no web fonts, no dependencies, strict CSP (no inline script/style; the JSON-LD block is data, which CSP allows). Keep its copy in step with the README, and the JSON-LD and `site/llms.txt` in step with the facts (price, requirements, tested cameras). `sitemap.xml` is generated by `build_site.sh`.
