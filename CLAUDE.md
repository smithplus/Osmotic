...
- Landing page interactions: hover only on `(hover: hover) and (pointer: fine)` and only a pixel or two of movement (a key rises off its skirt, panels and screenshots lift, an icon pocket lights its glyph, a table row lights its segments, the wordmark's dot glows). Scroll: a 2-px amber meter across the top edge, the key of the section in view latches like the app's tabs, the hero shot trails by up to 14 px, and readouts with `data-count` count up once. Everything drops its movement under Reduce Motion.
