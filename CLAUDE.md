...
- Landing videos: the hero (a session) and the Live feature (record, stop, modes) loop while in view, with no visible controls; install step 1 is a clip that plays once. Files and Webcam stay stills (the hero already shows Files; the Webcam screen doesn't move).
