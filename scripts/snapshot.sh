...
#   OSMOTIC_LANG=es scripts/snapshot.sh …   renders in another language (OSMOTIC_LOCALE=en_US for number formats)
set -euo pipefail
...
OSMOTIC_DEMO_THUMBS="${OSMOTIC_DEMO_THUMBS:-}" OSMOTIC_DEMO_MANIFEST="$MANIFEST" OSMOTIC_DEMO_SCREEN="$SCREEN" OSMOTIC_SNAPSHOT="$OUT" OSMOTIC_SNAPSHOT_QUIT=1 \
  "$ROOT_DIR/build/Osmotic.app/Contents/MacOS/Osmotic" ${OSMOTIC_LANG:+-AppleLanguages "($OSMOTIC_LANG)"} ${OSMOTIC_LOCALE:+-AppleLocale "$OSMOTIC_LOCALE"} >/dev/null 2>&1 &
PID=$!
